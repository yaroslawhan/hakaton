...
def start(message):
    ...
    if (message.text == "/start" or message.text == "Привет" or message.text == "привет") and isTraining == False:
        ...
        keyboard.add(key_go)
        key_stat= types.InlineKeyboardButton(text='Статистика', callback_data='stat')
...
def callback_worker(call):
    ...
    if call.data == "settings":
        ...
        key_back = types.InlineKeyboardButton(text='Назад', callback_data='ch_back');
    ...
    if call.data == "stat":
        ...
        for i in range(0, len(results)):
            if i == len(results) - 1:
                ...
                bot.send_message(call.message.chat.id, results[i].name + " " + str(results[i].score), reply_markup = keyboard)

                break

            bot.send_message(call.message.chat.id, results[i].name + " " + str(results[i].score))

    if call.data == "ch_back":
        messages = ['Вас приветствует лучший в мире бот для тренировки печати!', 'Hello, World!', 'Здравствуйте!', 'Привет, человек по ту сторону экрана.', 'С Наступающим Новым Годом!']
        i = random.randrange(0, 4)
        keyboard = types.InlineKeyboardMarkup()
        key_go= types.InlineKeyboardButton(text='Начать тренировку!', callback_data='go')
        ...
        key_settings = types.InlineKeyboardButton(text='Настройки', callback_data='settings');
        keyboard.add(key_settings);
        bot.send_message(call.message.chat.id, "{} Пожалуйста, выберите действие.".format(messages[i]), reply_markup = keyboard)
